...
class UtilBootstrapTable {
    ...
    public function search(array $fields = [], $keyword = "")
    {
        ...
        {
            $bufferFields = $fields;
            $this->db//->where($this->attachTableToThisField(array_pop($bufferFields)), 'LIKE', "%$keyword%") // bug need to use or where
    ...
    public function setSortBy($sort, $order = 'asc')
    {
        if(strlen($sort)>0)
        {
            $this->db->orderBy($this->attachTableToThisField($sort), $order);
        }
        return $this;
    }
